"use client"; // This is a client component
import { CUST_REVIEWS } from "@/constants";
import React, { useState } from "react";

const CustReviews = () => {
  return (
    <section className="flexCenter flex-col pt-10 bg-[#EDF4ED]">
      <div className="padding-container max-container w-full lg:px-72 px-10">
        <h2 className="text-center text-4xl font-bold mb-4 text-[24px] lg:text-[38px]">
          Customer Reviews
        </h2>

        <div>
          {CUST_REVIEWS.map((custReviewsList) => (
            <CustReviewsListItem
              key={custReviewsList.title}
              star={custReviewsList.star}
              title={custReviewsList.title}
              desc={custReviewsList.desc}
              point={custReviewsList.point}
            />
          ))}
        </div>
      </div>
    </section>
  );
};

type CustReviewsListItem = {
  point: string;
  star: string;
  title: string;
  desc: string;
};

const CustReviewsListItem = ({
  point,
  star,
  title,
  desc,
}: CustReviewsListItem) => {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="max-w-screen-xl px-5min-h-sceen bg-[#EDF4ED]">
      <div className="">
        <div className="flex items-center justify-center">
          <div className="bg-[#EDF4ED]">
            <div className="mt-4 text-left">
              <div className="flex mt-2 items-center border-b-[1px] border-gray-500  mb-5">
                <p className="mx-2 my-2 text-[#265C26]">{point}</p>

                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 text-green"
                  viewBox="0 0 20 20"
                  fill="#265C26"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 text-green"
                  viewBox="0 0 20 20"
                  fill="#265C26"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 text-green"
                  viewBox="0 0 20 20"
                  fill="#265C26"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 text-green"
                  viewBox="0 0 20 20"
                  fill="#265C26"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-4 w-4 text-green"
                  viewBox="0 0 20 20"
                  fill="#265C26"
                >
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              </div>

              <div className="mx-5">
                <p>{title}</p>
                <p
                  className={`mt-4 text-md text-gray-600 ${
                    isOpen ? null : "line-clamp-3"
                  }`}
                >
                  {desc}
                </p>

                <div className="flex justify-end">
                  <button onClick={() => setIsOpen(!isOpen)}>
                    {isOpen ? (
                      <svg
                        fill="none"
                        height="24"
                        shape-rendering="geometricPrecision"
                        stroke="currentColor"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="1.5"
                        viewBox="0 0 24 24"
                        width="24"
                      >
                        <path d="M18 15l-6-6-6 6"></path>
                      </svg>
                    ) : (
                      <svg
                        fill="none"
                        height="24"
                        shape-rendering="geometricPrecision"
                        stroke="currentColor"
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        stroke-width="1.5"
                        viewBox="0 0 24 24"
                        width="24"
                      >
                        <path d="M6 9l6 6 6-6"></path>
                      </svg>
                    )}
                  </button>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default CustReviews;
